...
export const selectTopProviders = (allPayments: AllPayment[], month: number): TopProvider[] => {
  const topProviders: TopProvider[] = [];
  const filteredPayments = allPayments.filter((payment) => payment.period_month === month);
  for (const payment of filteredPayments) {
    if (!topProviders.some((provider) => provider.name === payment.legal_entity_name)) {
      topProviders.push({
        name: payment.legal_entity_name,
        amount: payment.pay_package,
      });
    } else {
      const providerIndex = topProviders.findIndex((provider) => provider.name === payment.legal_entity_name);
  ...
  for (const reimbursement of filteredReimbursements) {
    if (!isMonthPresent(res, reimbursement.period_month)) {
      res.push({
        ...
        amount: 0,
        topProviders: selectTopProviders(reimbursements, reimbursement.period_month),
      })
    } else {
